...
def generate_modified_block_and_hash(block_file_path, variable_str, invalid_arg_type, conn=None):
    """
    ...
    if variable_str.lower() not in variables_list:
        ...
        if variable_str.lower() == variables_list[0]:
            updater.update_block_variables_using_invalid_version_data(new_block, invalid_arg_type, conn=conn)
            ...
            updater.update_block_variables_with_an_invalid_timestamp(new_block, invalid_arg_type, conn=conn)
            ...
            updater.update_block_variables_with_an_invalid_nonce(new_block, invalid_arg_type, conn=conn)
