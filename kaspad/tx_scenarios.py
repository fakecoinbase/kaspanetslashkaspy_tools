...
from kaspy_tools.kaspad.kaspa_dags.dnld_blocks_and_utxo_set_command import download_blocks_and_utxo_set
...
def generate_transactions_from_dag(*, addr_count=100, tx_count=100, block_count=3, conn=None):
    ...
    # download all blocks
    utxo_list, v_blocks, r_blocks = download_blocks_and_utxo_set(block_count, conn=conn)
...
def generate_double_spend_tx_pair(*, conn=None):
    ...
    addresses[mining_address.get_address(prefix='kaspadev')] = mining_address
    # download all blocks
    utxo_list, v_blocks, r_blocks = download_blocks_and_utxo_set(block_count=10, conn=conn)
